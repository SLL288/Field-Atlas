...
Geometry recovery now retains known valid polygon parts and unions overlapping valid components, with explicit partial-coverage warnings. See [geometry recovery](docs/GEOMETRY_REPAIR.md) for polygon repair, conversion of collapsed polygons to reference points/lines, and deployment requirements.

## Measure distance

Choose **Measure distance** on the map, then click/tap successive vertices. The total is the sum of geodesic segment lengths in metres/kilometres, not a road route or terrain/elevation distance. Undo removes the last vertex; Clear removes the sketch. While measuring, clicks add vertices instead of selecting licences. **Finish & save** leaves the line visible and archives it as a plot named `Distance measurement` with `distance_m` in its properties. Offline completion uses the existing upload queue. Unfinished sketches are temporary and are not saved on each click.
